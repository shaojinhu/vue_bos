<template>
    <div>
        <el-dialog title="编辑角色" :visible.sync="dialogFormVisible" @close="()=>{}" 
...
import { updateRole } from "@/api/system/role.js"
...
export default {
        ...
        role:function(val){
            ...
            if(val.permissions.length > 0){
                ...
                this.role.permissions.forEach(item=>{
                    this.permissionList.push(item.pid);
                    if(item.ptype != 1){//遍历掉主节点的选中状态
                        this.newPermission.push(item.pid)
                    }
                })
                console.log("push完成");
            }
    ...
    updated(){
        this.$refs.tree.setCheckedKeys(this.newPermission,false);
    }
}
</script>
